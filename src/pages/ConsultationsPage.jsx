  ...
  return (
    <div className="animate-fade">
      <div className="page-header">
        <div>
          <h1 className="page-title">{isPatient ? t('consultations.titlePatient', 'Мои консультации') : t('consultations.titleDoctor', 'Консультации')}</h1>
          <p className="page-subtitle">{t('consultations.records', '{{count}} записей', { count: items.length })}</p>
        </div>
        {!isPatient && (
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-primary" onClick={() => navigate('/record')}>
              ...
              {t('consultations.newConsultation', 'Новый приём')}
            </button>
          </div>
        )}
      </div>
      ...
      {!isPatient && (
        <div className="filter-bar">
          <div className="search-bar" style={{ flex: 1, maxWidth: 320 }}>
            ...
            <input className="input" placeholder={t('consultations.searchPlaceholder', 'Поиск по пациенту или ID...')} value={search} onChange={(e) => setSearch(e.target.value)} style={{ paddingLeft: 40 }} />
          </div>
          {STATUSES.map((s) => (
            <button key={s} className={`filter-chip ${filter === s ? 'active' : ''}`} onClick={() => setFilter(s)}>
              {s ? statusLabels[s] : t('consultations.all', 'Все')}
            </button>
          ))}
        </div>
      )}
      ...
      {loading ? <div className="loading-spinner" /> : filtered.length === 0 ? (
        <div className="card empty-state">
          <h3>{t('consultations.emptyTitle', 'Нет консультаций')}</h3>
          <p>{isPatient ? t('consultations.emptyPatient', 'У вас пока нет записей о консультациях') : t('consultations.emptyDoctor', 'Создайте новый приём для начала работы')}</p>
        </div>
      ) : (
          ...
          {filtered.map((c) => (
            <div key={c.id} className="card consult-card" onClick={() => navigate(`/consultations/${c.id}`)}>
              <div className="consult-card-header">
                ...
                <span className={`badge ${STATUS_COLORS[c.status]}`}>{statusLabels[c.status] || c.status}</span>
              </div>
              <div className="consult-card-meta">
                ...
                {c.images_count > 0 && (
                  <span className="consult-card-meta-item">
                    ...
                    {t('consultations.images', '{{count}} снимков', { count: c.images_count })}
                  </span>
                )}
              </div>
            </div>
