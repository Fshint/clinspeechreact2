  ...
  return (
    <div className="animate-fade">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">Добро пожаловать, {displayName}!</h1>
          <p className="page-subtitle">Обзор активности и статистики</p>
        </div>
      ...
      {!isPatient && stats && (
        <div className="stat-grid">
          <div className="card stat-card">
            ...
            <div className="stat-card-label">Всего консультаций</div>
          </div>
          <div className="card stat-card">
            ...
            <div className="stat-card-label">Завершённых</div>
          </div>
          <div className="card stat-card">
            ...
            <div className="stat-card-label">В этом месяце</div>
          </div>
          <div className="card stat-card">
            ...
            <div className="stat-card-label">Пациентов</div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16, marginBottom: 24 }}>
        <div className="card" style={{ padding: 24 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 16 }}>Активность за 14 дней</h3>
          {activityData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={activityData}>
                ...
                <Tooltip />
                <Area type="monotone" dataKey="count" stroke="#2EC4B6FF" fill="url(#gradActivity)" strokeWidth={2} name="Консультаций" />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
        ...
        <div className="card" style={{ padding: 24 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 16 }}>По статусам</h3>
          {stats && (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={Object.entries(statusData).map(([k, v]) => ({ name: STATUS_LABELS[k] || k, value: v }))}>
                <XAxis dataKey="name" tick={{ fontSize: 10 }} />
                <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                <Tooltip />
                <Bar dataKey="value" fill="#2EC4B6FF" radius={[4, 4, 0, 0]} name="Кол-во" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isPatient ? '1fr' : '1fr 1fr', gap: 16 }}>
        <div className="card" style={{ padding: 0 }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ fontSize: 16, fontWeight: 600 }}>Последние консультации</h3>
            <button className="btn btn-ghost btn-sm" onClick={() => navigate('/consultations')}>Все →</button>
          </div>
            ...
            recentConsults.map((c) => (
              <div key={c.id} style={{ padding: '12px 20px', borderBottom: '1px solid var(--border-light)', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} onClick={() => navigate(`/consultations/${c.id}`)}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{new Date(c.created_at).toLocaleDateString('ru-RU')}</div>
                </div>
                <span className={`badge ${STATUS_COLORS[c.status] || 'badge-neutral'}`}>{STATUS_LABELS[c.status] || c.status}</span>
              </div>
            ))
          )}
        </div>
        ...
        {!isPatient && (
          <div className="card" style={{ padding: 0 }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ fontSize: 16, fontWeight: 600 }}>Приёмы на сегодня</h3>
              <button className="btn btn-ghost btn-sm" onClick={() => navigate('/appointments')}>Все →</button>
            </div>
            {todayAppts.length === 0 ? (
              <div className="empty-state"><p>На сегодня приёмов нет</p></div>
            ) : (
              todayAppts.map((a) => (
                <div key={a.id} style={{ padding: '12px 20px', borderBottom: '1px solid var(--border-light)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{new Date(a.scheduled_at).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })} · {a.reason || 'Приём'}</div>
                  </div>
                  <span className={`badge ${a.status === 'scheduled' ? 'badge-info' : a.status === 'completed' ? 'badge-success' : 'badge-neutral'}`}>{a.status === 'scheduled' ? 'Ожидается' : a.status === 'completed' ? 'Завершён' : a.status}</span>
                </div>
              ))
            )}
          </div>
